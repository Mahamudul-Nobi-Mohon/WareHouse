...
namespace WMS
{
    public partial class AddCell : Form
    {
        ...
        public void DataGrid()
        {
            ...
            SqlCommand cmd = con.CreateCommand();
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = "SELECT LocationSub.LocationSubID AS 'ID', LocationMain.LocationMainName AS 'Location Main Name',Location.LocationName AS 'Location Name', LocationSub.LocationSubName  AS 'Location Sub Name'  FROM LocationSub  INNER JOIN LocationMain    ON LocationSub.LocationMainID = LocationMain.LocationMainID INNER JOIN Location ON Location.LocationID = LocationSub.LocationID";
            cmd.ExecuteNonQuery();
            DataTable dt = new DataTable();
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            da.Fill(dt);
